...
X_TRAIN = r"data/cleaned/first_row_x_test_dic.csv"
PLOT = r"res"
...
x_int_dic_eyy = []
x_int_dic_exy = []
with open(os.path.join(DATA, f"x_test_dic_30_multiquadric.csv"), 'r') as file:
    reader = csv.reader(file)
    header = next(reader)
    row = next(reader)
    for i in range(len(row) - (int(len(row) / 20) - 2), len(row), 3):
...
for grid in GRIDS:
    ...
    for method in METHODS:
        ...
        # --- color scale for diffs (column 3) ---
        diff_min = 0.0
        diff_max = max(np.nanmax(x_diff_exx), np.nanmax(x_diff_eyy), np.nanmax(x_diff_exy))
        ...
        data_pairs = [
            (x_int_exx, x_int_dic_exx, x_diff_exx),
            (x_int_eyy, x_int_dic_eyy, x_diff_eyy),
            (x_int_exy, x_int_dic_exy, x_diff_exy)
        ]
        ...
        for row_idx, (x_int_left, x_int_dic, x_diff) in enumerate(data_pairs):
            ...
            # Mid column (DIC)
            scatter_dic = axes[row_idx, 1].scatter(
                x_coords, y_coords, c=x_int_dic, cmap='jet', s=size, alpha=0.9, edgecolors='none',
                vmin=global_min, vmax=global_max, rasterized=True
            )
            # axes[row_idx, 1].set_title(f'{titles[row_idx]} - Original')
            axes[row_idx, 1].set_xlabel('x [mm]')
            ...
            scatter_diff = axes[row_idx, 2].scatter(
                x_coords, y_coords, c=x_diff, cmap='Greys', s=size, alpha=0.9, edgecolors='none',
        ...
        # Colorbar for strain fields (columns 1–2)
        cax1 = fig.add_axes([0.10, 0.035, 0.55, 0.02])  # spans first two columns
        cbar1 = fig.colorbar(scatter_dic, cax=cax1, orientation='horizontal')
        cbar1.set_ticks(np.linspace(global_min, global_max, num=7))

        # Colorbar for diff field (column 3)
        cax2 = fig.add_axes([0.72, 0.035, 0.23, 0.02])  # aligned under third column
        cbar2 = fig.colorbar(scatter_diff, cax=cax2, orientation='horizontal')
        cbar2.set_ticks(np.linspace(diff_min, diff_max, num=5))

        # saves and closes plot
        plt.savefig(os.path.join(PLOT, f'interp_{grid}_{method}_3.pdf'), dpi=300)
        plt.close()
